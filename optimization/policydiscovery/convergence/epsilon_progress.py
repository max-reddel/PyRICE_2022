...
def visualize_epsilon_progress(saving=False):
    ...
    # Load outcomes
    df_con1 = pd.read_csv(
        directory + "/results_formatted/NORDHAUS_UTILITARIAN_200000_convergence.csv"
    )
    df_con2 = pd.read_csv(
        directory + "/results_formatted/NORDHAUS_SUFFICIENTARIAN_200000_convergence.csv"
    )
    df_con3 = pd.read_csv(
        directory + "/results_formatted/WEITZMAN_UTILITARIAN_200000_convergence.csv"
    )
    df_con4 = pd.read_csv(
        directory + "/results_formatted/WEITZMAN_SUFFICIENTARIAN_200000_convergence.csv"
    )
    ...
    if saving:
        ...
        fig.savefig(
            visualization_folder + "convergence_epsilon_progress.png",
            dpi=200,
            pad_inches=0.2,
        )
